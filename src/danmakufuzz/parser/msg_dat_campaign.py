...
from .msg_dat import parse_msg_dat
from .msg_dat_mutants import MsgDatMutant, generate_msg_dat_mutants


DEFAULT_ARCHIVE = REFERENCE_DIR / "retail" / "game" / "th06" / "紅魔郷ST.DAT"
DEFAULT_ENTRY = "msg1.dat"


def _default_artifact_dir() -> Path:
    stamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    return ARTIFACTS_DIR / "parser-msg-dat" / stamp


def _baseline_summary(seed_payload: bytes, source: str, *, max_message_instructions: int) -> dict[str, object]:
    return {
        "input": source,
        **parse_msg_dat(seed_payload, max_message_instructions=max_message_instructions),
    }


def _message_digest(summary: dict[str, object]) -> dict[str, object]:
    ...
    return {
        "count": len(messages),
        "instruction_counts": [
            message.get("instruction_count") if isinstance(message, dict) else None
...
def evaluate_msg_dat_payload(payload: bytes, baseline_summary: dict[str, object], *, max_message_instructions: int) -> dict[str, object]:
    try:
        parsed = parse_msg_dat(payload, max_message_instructions=max_message_instructions)
    ...
    parsed_digest = _message_digest(parsed)
    baseline_digest = _message_digest(baseline_summary)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run a lightweight Touhou message DAT mutation campaign over one stage-message seed."
    )
...
def main() -> int:
    ...
    baseline = _baseline_summary(payload, source, max_message_instructions=args.max_message_instructions)
    (artifact_dir / "baseline.json").write_text(json.dumps(baseline, indent=2) + "\n", encoding="utf-8")
    mutants = generate_msg_dat_mutants(payload)
    if args.name_filter:
        mutants = [
            mutant
    ...
    with summary_path.open("w", encoding="utf-8") as summary_handle:
        for case_index, mutant in enumerate(mutants, start=1):
            result = _run_mutant(
                ...
                max_message_instructions=args.max_message_instructions,
            )
    ...
    report = {
        "schema": "danmakufuzz-msg-dat-campaign-v1",
        "input": source,
