...
TARGET_MUTANT_NAME = "first-entry-size-inflate"
EXPECTED_PAYLOAD_SHA256 = "a4235974801b57820b17d2831da10b3c6a54e833df5fa35cb51eba26a24a0ee3"
...
EXPECTED_BASELINE_UNCOMPRESSED_SIZE = 2100
EXPECTED_MUTANT_UNCOMPRESSED_SIZE = 3124
EXPECTED_ERROR_TYPE = "Pbg3Error"
EXPECTED_ERROR_MESSAGE = "PBG3 size mismatch for stg1bg.anm: 2100 != 3124"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Reproduce the PBG3 first-entry-size-inflate mismatch finding."
    )
    parser.add_argument("--archive", type=Path, default=DEFAULT_ARCHIVE)
    parser.add_argument(
        "--artifact-dir",
        type=Path,
        default=ARTIFACTS_DIR / "findings" / "parser-pbg3-first-entry-size-inflate-mismatch",
    )
...
def main() -> int:
    ...
    if first_baseline_entry.uncompressed_size != EXPECTED_BASELINE_UNCOMPRESSED_SIZE:
        raise RuntimeError(
            f"baseline first entry size drifted: expected {EXPECTED_BASELINE_UNCOMPRESSED_SIZE}, "
    ...
    if first_entry.uncompressed_size != EXPECTED_MUTANT_UNCOMPRESSED_SIZE:
        raise RuntimeError(
            f"mutant first entry size drifted: expected {EXPECTED_MUTANT_UNCOMPRESSED_SIZE}, "
    ...
    summary = {
        "finding": "parser/pbg3-first-entry-size-inflate-mismatch",
